import{P as w,c as u,g as L,a as y,s as m}from"./main-start-ce8904ef.js";function q(n,d,$,x,o){let s=x===6?6:L(),N=3;n.length<=s&&document.querySelector(".tui-pagination").classList.add("visually-hidden");const g=document.getElementById("tui-pagination-container");new w(g,{totalItems:n.length,itemsPerPage:s,visiblePages:N}).getCurrentPage();let k=Array.from(document.querySelectorAll(".tui-page-btn"));Number.isInteger(o)===!0&&k.forEach(t=>{Number(t.textContent)===o&&t.click()});const r=()=>{let t=[],a=s,e=0;for(let i=0;i<Math.ceil(n.length/s);i++){for(t.push([]),e;e<a&&n[e]!==void 0;e++)t[i].push(n[e]);a+=s}return t};let c;const C=t=>(t=t.toLowerCase(),t==="yes"?"Alcoholic":t==="no"?"Non-Alcoholic":"NA"),l=t=>{const a=r()[t].map(e=>{const{_id:i,drink:b,drinkThumb:h,description:v}=e;return d==="byName"||d==="byAbc"?c=`
                ...
                </li>`,c}).join("");document.querySelector(`.${$}`).innerHTML=a},f=Number.isInteger(o)===!0?o-1:0;l(f),u(r()[f]);const p=()=>{Array.from(document.querySelectorAll(".tui-page-btn")).forEach(a=>{Number.isInteger(Number(a.textContent))&&a.classList.add("tui-number-item")})};p();function A(t){p();try{if(t.target===t.currentTarget)return;(t.target.classList.contains("tui-prev")||t.target.classList.contains("tui-ico-prev"))&&(l(Number(document.querySelector(".tui-is-selected").textContent)-1),u(r()[Number(document.querySelector(".tui-is-selected").textContent)-1])),(t.target.classList.contains("tui-next")||t.target.classList.contains("tui-ico-next")&&t.target.classList.contains("tui-is-disabled")===!1)&&(l(Number(document.querySelector(".tui-is-selected").textContent)-1),u(r()[Number(document.querySelector(".tui-is-selected").textContent)-1])),t.target.classList.contains("tui-number-item")&&(l(Number(t.target.textContent)-1),u(r()[Number(t.target.textContent)-1]))}catch{}}g.addEventListener("click",A)}const P="/js-89-sidlicki/assets/coctail-27293f2f.webp",I="/js-89-sidlicki/assets/coctail@2x-7864576c.webp";export{I as a,P as n,q as p};
